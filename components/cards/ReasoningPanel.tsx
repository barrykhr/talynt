      ...
      <ul className="mt-5 flex flex-col gap-4">
        {candidate.reasoning.map((line, i) => (
          <li key={i} className="flex gap-3.5">
            ...
            />
            <span className="text-[0.9rem] leading-[1.6] opacity-85">{line}</span>
          </li>
        ))}
      </ul>

      <div className="mt-6 border-t border-signal/20 pt-5">
        <p className="mono-micro mb-2.5 opacity-50">What to watch</p>
        <p className="text-[0.88rem] leading-[1.6] opacity-70">
          {candidate.watchFor}
        </p>
      </div>
    </section>
